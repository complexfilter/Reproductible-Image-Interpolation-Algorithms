function y = perform_wavelet_transform_1scale(x, dir, options)
...
if ( ndim ~= 2 )
    error('The input should be a 2D image matrix');
end
...
if ti
    if dir==1
        y = FWT2_TI_1scale(x,qmf);
    else
        y = IWT2_TI_1scale(x,qmf);
    end
    return;
end
...
function wc = FWT2_PO_1scale(x,qmf)
...
function x = IWT2_PO_1scale(wc,qmf)
...
top = (n1/2+1):n1; bot = 1:(n1/2); all = 1:n1;
for iy=1:n2
    x(all,iy) =  UpDyadLo(x(bot,iy)',qmf)'+ UpDyadHi(x(top,iy)',qmf)';
end
top = (n2/2+1):n2; bot = 1:(n2/2); all = 1:n2;
for ix=1:n1
...
function tiwt = FWT2_TI_1scale(x,qmf)
...
%     s = tiwt(3*D*n+packet(d,b1,n), packet(d,b2,n));
    ...
    wc00 = FWT2_PO_1scale(s,qmf);
    wc01 = FWT2_PO_1scale(CircularShift(s,0,1),qmf);
    wc10 = FWT2_PO_1scale(CircularShift(s,1,0),qmf);
    wc11 = FWT2_PO_1scale(CircularShift(s,1,1),qmf);
...
%     index11 = packet(d+1,2*b1+1,n); index21 = packet(d+1,2*b2+1,n);
    ...
    % vertical stuff
    tiwt(index10 , index20) = wc00(1:(n1/2),(n2/2+1):n2);
    tiwt(index11 , index20) = wc01(1:(n1/2),(n2/2+1):n2);
    tiwt(index10 , index21) = wc10(1:(n1/2),(n2/2+1):n2);
    tiwt(index11 , index21) = wc11(1:(n1/2),(n2/2+1):n2);
    % horizontal stuff
    tiwt(n1+index10 , index20) = wc00((n1/2+1):n1,1:(n2/2));
    tiwt(n1+index11 , index20) = wc01((n1/2+1):n1,1:(n2/2));
    tiwt(n1+index10 , index21) = wc10((n1/2+1):n1,1:(n2/2));
    tiwt(n1+index11 , index21) = wc11((n1/2+1):n1,1:(n2/2));
    % diagonal stuff
    tiwt(2*n1+index10, index20) = wc00((n1/2+1):n1,(n2/2+1):n2);
    tiwt(2*n1+index11, index20) = wc01((n1/2+1):n1,(n2/2+1):n2);
    tiwt(2*n1+index10, index21) = wc10((n1/2+1):n1,(n2/2+1):n2);
    tiwt(2*n1+index11, index21) = wc11((n1/2+1):n1,(n2/2+1):n2);
    % low freq stuff
    tiwt(3*n1+index10, index20) = wc00(1:(n1/2),1:(n2/2));
    tiwt(3*n1+index11, index20) = wc01(1:(n1/2),1:(n2/2));
    tiwt(3*n1+index10, index21) = wc10(1:(n1/2),1:(n2/2));
    tiwt(3*n1+index11, index21) = wc11(1:(n1/2),1:(n2/2));
% 	    end, end
% 	end
...
function x = IWT2_TI_1scale(tiwt,qmf)
...
%
% 	lastx = (3*D*n+1):(3*D*n+n); lasty = 1:n;
	lastx = (3*n1+1):(3*n1+n1); lasty = 1:n2;
	x = tiwt(lastx,lasty);
...
% 	      index11 = packet(d+1,2*b1+1,n); index21 = packet(d+1,2*b2+1,n);
        ...
        index10 = 1 : n1/2; index20 = 1 : n2/2;
        index11 = n1/2+1 : n1; index21 = n2/2 + 1 : n2;
              
	      wc00 = [x(index10,index20) tiwt(index10,index20) ; ...
		tiwt(n1+index10,index20) tiwt(2*n1+index10,index20)];
	      wc01 = [x(index11,index20) tiwt(index11,index20) ; ...
		tiwt(n1+index11,index20) tiwt(2*n1+index11,index20)];
	      wc10 = [x(index10,index21) tiwt(index10,index21) ; ...
		tiwt(n1+index10,index21) tiwt(2*n1+index10,index21)];
	      wc11 = [x(index11,index21) tiwt(index11,index21) ; ...
		tiwt(n1+index11,index21) tiwt(2*n1+index11,index21)];
% 
...
% 		CircularShift(IWT2_PO(wc11,l,qmf),-1,-1) ) / 4;
        ...
        x = ( IWT2_PO_1scale(wc00,qmf) + ....
            CircularShift(IWT2_PO_1scale(wc01,qmf),0,-1) + ...
            CircularShift(IWT2_PO_1scale(wc10,qmf),-1,0) + ...
            CircularShift(IWT2_PO_1scale(wc11,qmf),-1,-1) ) / 4;
% 	  end, end
% 	end
