...
%  f = double(imread('pepper.png'));
 f = double(imread('11.gif'));
% f = double(imread('elk_gray.bmp'));
% f = double(rgb2gray(imread('dragonfly.jpg')));

% f = double(rgb2gray(imread('fighter.jpg')));
...
%    (26.5651,129,129,257,257,4,200,0,0,2);% ground truth image. 
% I_ini=edge_generate_straight_offset_0131...
...
%     ind_shift
    for j = 1:ScaleNumber
        ...
        for k = 1 : BNumber % aggregate 28 types of blocks into 20 direction range.
            ...
            if k==1
            disp([k1 k EpsilonMapLR(144,177) AngleMapLR(144,177)  (M_a(144,177,1)) ]);
            end
            if k==2
            disp([k1 k EpsilonMapLR(144,177) AngleMapLR(144,177)  (M_a(144,177,2)) ]);
            end
        end  
    end   
...
option.lambda = 0.6;
option.T = 0;
[imH_SRRR, imH_linear] = SME_ImageZoom(imL, option);
