function y=RMLS_Interpolation(x,options)   % 2X enlarge 
...
if count>1  
  if var(unlabel_neighbor)<options.th 
     ...
     wi = [0.25,0.25,0.25,0.25,0];
      ...
      center = Eim(i-swin:i+swin,j-swin:j+swin);%�ڸ߷ֱ���ͼ����ȡֵ
      ...
      for ii=i-2*(win-1)-1:2:i+2*win
          for jj=j-2*(win-1)-1:2:j+2*win
             sample =  Eim(ii-2*swin:2:ii+2*swin,jj-2*swin:2:jj+2*swin);%����label sample��similar win���ڵͷֱ���ͼ��ȡ��
